...
\begin{itemize}
    \item \textbf{Nombre del proyecto:} Juego Dodger en ESP32 con pantalla OLED
    \item \textbf{Integrantes del equipo:} Sara Rondon, Natalia Ángel
    \item \textbf{Asignatura:} Electrónica Digital II
\end{itemize}

\section{Resumen}
El proyecto consiste en el desarrollo de un videojuego embebido tipo \textit{Dodger}, implementado sobre un microcontrolador ESP32 y una pantalla OLED SSD1306 de 128×64 píxeles, programado en MicroPython.
...
\textbf{Entradas}
\begin{itemize}
    \item Pulsador UP (GPIO 32): movimiento del jugador hacia arriba.
    \item Pulsador DOWN (GPIO 33): movimiento del jugador hacia abajo.
    \item Pulsador START/PAUSE (GPIO 25): inicio, pausa y reanudación del juego.
\end{itemize}

\textbf{Procesamiento}
\begin{itemize}
    \item Comunicación I2C para el control de la pantalla OLED.
    \item Lógica de juego basada en máquina de estados.
    \item Generación de obstáculos, detección de colisiones y cálculo de puntaje.
\end{itemize}

\textbf{Salidas}
\begin{itemize}
    \item Pantalla OLED SSD1306 (128×64 píxeles).
    \item Buzzer pasivo controlado por PWM (GPIO 26).
\end{itemize}
...
\begin{figure}
        \centering
        \includegraphics[width=0.5\linewidth]{DIAGRAMA SEGUI4.png}
...
El software está desarrollado en MicroPython, utilizando los módulos \texttt{machine}, \texttt{ssd1306}, \texttt{framebuf}, \texttt{utime} y \texttt{random}.
...
\begin{itemize}
    \item Configuración de pines y parámetros globales.
    \item Inicialización del hardware (OLED, botones y buzzer).
    \item Definición de sprites y framebuffers.
    \item Clases para jugador, obstáculos y dificultad.
    \item Máquina de estados del juego.
    \item Bucle principal de ejecución.
\end{itemize}


\section{Explicación de funciones principales}
\begin{itemize}
    \item \texttt{buzzer\_tone():} Genera señales de audio mediante PWM como retroalimentación sonora.
    \item \texttt{DebouncedButton.update():} Implementa antirrebote por software para los pulsadores.
    \item \texttt{aabb():} Realiza la detección de colisiones mediante bounding boxes.
    \item \texttt{Player.move\_up() / move\_down():} Controlan el desplazamiento vertical del jugador.
    \item \texttt{Obstacle.update():} Actualiza la posición y velocidad de los obstáculos.
    \item \texttt{Game.update():} Controla la lógica principal del juego y las condiciones de finalización.
    \item \texttt{Game.draw():} Renderiza los elementos gráficos según el estado del sistema.
\end{itemize}
...
El sistema emplea bloques \texttt{try/except} para manejar interrupciones inesperadas, garantizando la desactivación del buzzer y la limpieza de la pantalla.
...
class Game:
    ...
    def update(self):
        ...
        self.survival_time = (utime.ticks_ms() - self.start_time)//1000
    ...
    def draw(self, d):
        ...
        if self.state == STATE_MENU:
            ...
            d.text("O:"+str(self.obstacles_dodged), 40, 0, 1)  # 
            d.text("T:"+str(self.survival_time), 90, 0, 1)     # 
            ...
            d.text("PAUSA", 40, 20, 1)
            d.text("S:"+str(self.score), 30, 40, 1)
            d.text("START=CONT", 10, 55, 1)
